...
class RegularClass(object):
  ...
  def test_testutils_should_match_expectations_against_builtin_classes(self):
    class Foo:
      def method_foo(self): pass
    mock = wrap(Foo)
    foo = Foo()
  ...
  def test_testutils_should_match_expectations_against_user_defined_classes(self):
    class Foo:
      def method_foo(self): pass
    foo = Foo
    mock = wrap(foo)
  ...
  def test_testutils_should_not_return_class_object_if_mocking_instance(self):
    class User:
  ...
  def test_testutils_should_blow_up_on_default_for_class_mock(self):
    class User:
      def foo(self):
        return 'class'
    assertRaises(TestutilsError, wrap(User).foo.calls_original)

  def test_testutils_should_not_blow_up_on_default_for_class_methods(self):
    class User:
      @classmethod
      def foo(self):
        return 'class'
  ...
  def test_testutils_should_not_blow_up_on_default_for_static_methods(self):
    class User:
      @staticmethod
      def foo():
        return 'static'
  ...
  def test_testutils_should_mock_new_instances_with_multiple_params(self):
    class User(object): pass
    class Group(object):
      def __init__(self, arg, arg2):
        pass
  ...
  def test_testutils_should_mix_multiple_return_values_with_exceptions(self):
    class Foo:
      def method1(self): pass
    foo = Foo()
  ...
  def test_testutils_should_verify_spy_matches_exception_regexp_mismatch(self):
    class User:
      def get_stuff(self): raise Exception('123asdf345')
    user = User()
    wrap(user).get_stuff.calls_original().raises(
  ...
  def test_testutils_should_properly_restore_undecorated_static_methods(self):
    class User:
      def get_stuff(): return 'ok!'
  ...
  def test_arg_matching_works_with_regexp(self):
    ...
    foo = Foo()
    wrap(foo).foo(re.compile('^arg1.*asdf$'), arg2=re.compile('f')).returns(
        'mocked')
  ...
  def test_arg_matching_with_regexp_fails_when_regexp_doesnt_match_karg(self):
    class Foo:
      def foo(arg1, arg2): pass
    foo = Foo()
    wrap(foo).foo( re.compile('^arg1.*asdf$'), arg2=re.compile('a')).returns(
        'mocked')
